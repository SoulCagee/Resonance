...
const MIN_AUDIO_DURATION_SECONDS = 9;
...
export async function POST(request: Request) {
  ...
  if (fileBuffer.byteLength > MAX_UPLOAD_SIZE_BYTES) {
    return Response.json({ error: 'Audio file exceeds the 20MB size limit' }, { status: 413 });
  }
  ...
  try {
    ...
    return Response.json({ error: 'File to create voice . please try again' }, { status: 500 });
  }
